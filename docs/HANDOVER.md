...
3. **开箱距离与寻路**：开箱按服务器原版判定——**玩家眼睛到箱子包围盒距离 ≤3 格（interaction range，代码留 0.1 余量按 2.9）**即可，与玩家手动操作等价，不必紧贴箱子。寻路先 GoalNear(1.5)、失败放宽到 3.0（悬空/高处的箱子最近站格常在正下方、距中心约 3.0 格，必须 ≥3 才能走到），再失败时若眼睛已在 reach 内仍直接开箱。**距离不够且站在地面时允许原地跳起开箱**（`setControlState('jump')`，眼睛随跳跃升高，用户 2026-08 授权；开箱结束自动停跳）。曾为 ≤1.6 格、后 2.0、再后中心距 3.0（边缘服务器拒收）、又收紧寻路上限 2.5（悬空箱 20+ 格远就放弃），最终：寻路上限 3.0 + 眼睛到包围盒判定 2.9 + 跳起开箱。**超距（>2.9）明确拒绝**。
